...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const res = await fetch("https://ecomercebackend-1.onrender.com/api/categories/admin/getallcategory");
        const data = await res.json();
  ...
  const handleDelete = async (id:any) => {
    try {
      const res = await fetch(`https://ecomercebackend-1.onrender.com/api/products/${id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (data.status === "success") {
  ...
  const handleAddProduct = async() => {
    ...
    try {
      const res = await fetch("https://ecomercebackend-1.onrender.com/api/products/add", {
        method: "POST",
        headers: {
